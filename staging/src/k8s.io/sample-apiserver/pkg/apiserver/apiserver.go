...
func (c completedConfig) New() (*WardleServer, error) {
	genericServer, err := c.Config.GenericConfig.SkipComplete().New("sample-apiserver", genericapiserver.EmptyDelegate, nil) // completion is done in Complete, no need for a second time
	if err != nil {
		return nil, err
	}
