...
func NewEtcd(alloc allocator.Snapshottable, baseKey string, resource schema.GroupResource, config *storagebackend.Config, stopCh <-chan struct{}) *Etcd {
	storage, _ := generic.NewRawStorage(config, stopCh)
	return &Etcd{
		alloc:    alloc,
		storage:  storage,
		baseKey:  baseKey,
