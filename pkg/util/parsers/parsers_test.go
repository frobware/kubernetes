...
func TestSplitImageName(t *testing.T) {
	for i, test := range []struct {
		...
		if err != nil && !test.expectErr {
			t.Fatalf("test #%v failed for input %q: %v", i, test.input, err)
		}
		if test.domain != domain {
			t.Errorf("test #%v failed; expected domain=%q, got domain=%q", i, test.domain, domain)
		}
		if test.remainder != remainder {
			t.Errorf("test #%v failed; expected remainder=%q, got remainder=%q", i, test.remainder, remainder)
		}
	}
}
